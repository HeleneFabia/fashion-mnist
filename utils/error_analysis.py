...
def get_top_X_wrong_predictions(X, preds, idx_false_preds):
    """ 
    ...
    for i in range(false_preds.shape[0]):
        ...
        true_label = test_ds_cnn.labels[idx_pred].item()
    ...
    show_false_predictions(test_ds_cnn.images[idx_top_X_false].reshape(X,28,28), int(X/5), 5, true=get_label(true_labels_false_sorted[:X]), 
